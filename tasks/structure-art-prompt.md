...
- `barricade.png` — Barricade — base `#8a6a3c` — rough scrap timber thrown
  together at angles, gaps visible, obviously the flimsiest of the set
- `woodWall.png` — Wood Wall — base `#a3763f` — neat horizontal wooden
  planks, solid and uniform
- `stoneWall.png` — Stone Wall — base `#8f8a80` — dry-stacked irregular grey
  stone blocks, no wood or metal anywhere
- `reinforcedWall.png` — Reinforced Wall — base `#9a8256` — wood planks with
  riveted sheet-metal plates bolted over them, visibly patched
- `metalWall.png` — Steel Wall — base `#9aa2ab` — heavy welded steel plate
  with rivets along the edges, the toughest-looking of the five
- `spike.png` — Spike Trap — base `#8a8078` — a bed of sharpened stakes
...
- `gate_closed.png` — Gate (closed) — base `#b08a5a` — a wooden gate barring
...
- `gate_open.png` — Gate (open) — base `#b08a5a` — the same gate swung open:
...
- `stash.png` — Supply Stash — base `#a3763f` — a large wooden supply crate
  seen from above, lid boards visible, the biggest storage piece
- `chest.png` — Wooden Chest — base `#8a6a3c` — a smaller wooden chest from
  above, visibly smaller and simpler than the stash
- `locker.png` — Steel Locker — base `#9aa2ab` — a grey steel cabinet from
  above, metal not wood, with a vent or handle detail
- `workbench.png` — Workbench — base `#b08a5a` — a wooden work surface from
...
- `chemStation.png` — Chemistry Station — base `#7fd0c4` — a lab bench from
...
- `bedroll.png` — Bedroll — base `#6f7a52` — a rolled-out sleeping bag /
  bedroll lying flat on the ground, seen from above, olive canvas
- `bunk.png` — Bunk — base `#6f7a52` — a simple wood-framed bunk bed from
  above: frame, mattress, pillow at one end
- `raisedBed.png` — Raised Bed — base frame `#5a4632` — **this one has a
...
- `watchtower.png` — Watchtower — base `#a3763f` — a wooden tower seen from
...
- `turret.png` — Auto Turret — base `#5e6a72` — **draw the base only, with
...
- `floodlight.png` — Floodlight — base `#c9a227` — a floodlight fixture on a
...
- `generator.png` — Generator — base `#71787f` — a small fuel generator unit
...
Send me the PNGs. Note that unlike items, **there is no existing code path
that loads structure art at all** — `src/world/structure_view.gd` draws
every buildable as a hand-coded rectangle from a hardcoded `COLORS` dict,
and `src/ui/build_bar.gd` does the same for the build menu. Adding these
sprites means writing that loader, keeping the code-drawn overlays (damage
tint, health bar, crop stages, turret barrel, powered rings) working on top
of the new art, and falling back to the current colored rectangle for
anything with no sprite yet.
