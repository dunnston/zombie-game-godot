...
| `tasks/port-inventory.md` | **History.** The checklist the port was measured against: every system the prototype had and its numbers then. The port is complete; the code and `data/` are the spec now. |
...
What each row was measured against is in `tasks/port-inventory.md` (history now).
...
| Player movement, stamina, camera | 1 | **redesigned** | Stamina slows you rather than refusing you: every swing costs, sprinting runs the bar flat, and winded stretches the swing (§ *Stamina*) |
...
gaps between them, and the gate looked wrong and faced the wrong way.

- **A wall or gate fills its tile and joins its neighbours.**
  `StructureView.joins(type)` is every `wall` piece and the gate, open or shut.
  `slice_world.py` cuts those pictures to the whole tile, stretched square — a
  wall to its body first (`WALL_BODY`), since the post stubs along its top
  would stand up out of a joined wall at every tile; the gate keeps its posts.
  `join_mask` reads the four neighbours, and `_draw_joined` trims the picture's
  outline (`JOIN_TRIM`, 5px of 128) off each side that meets another piece of
  wall, so a row of mixed walls reads as one wall with one outline round it.
  Other pieces are fitted as before.
- **A gate turns with its wall.** Square pieces never turn (`rot` is always
  0), so the gate follows its neighbours instead: wall above or below and
  none beside, and it is drawn a quarter turn round (`gate_turned`). Drawing
  only — nothing in the sim, the wire or the save changed. A wall's picture
  never turns; its boards run on either way.
- The build ghost of a wall or gate joins the wall it would stand in.
- The smoke's `structure_art` layout turns its wall row down a corner with a
  gate in it, and asserts the row's join masks and that the row gate lies flat
  and the column gate turned.

### Built pieces look like themselves, and a bed two tiles long (2026-09-15)

The owner added a folder of top-down pictures of every buildable — the
things standing in the street, not the build-menu icons, which stay as they
were — and a long raised bed with them.

- **`art/world/<id>.png` is how a piece looks built.** `Structures.world_art_of`
  is the same seam as `icon_of`: no file, and `StructureView` draws the piece
  in code as before. A picture is fitted to the piece's tiles without
  stretching. Two pieces have a second look, `<id>_<state>.png` from
  `Structures.WORLD_STATES`: `gate_open`, and `turret_head`. An open gate with
  no picture of its own is drawn in code, never as a shut one.
- **The state is still drawn, over the picture.** Damage darkens it (as a
  modulate) and the bar shows above it; a hit flashes it; a bed's wet soil
  darkens, and its shoots and ready ring are drawn as ever; the generator's
  ring, the floodlight's, the active bedroll's, Workbench II's label, NO 9MM
  and NO POWER all stay. The turret is the sheet's mount standing still with
  `turret_head` over it, 1.4 tiles across, turned to `aim` and grey without
  power. The build ghost shows the picture faintly, lying the way it will go.
  This replaces "the street still draws structures in code" from the icon
  set: the pictures that came this time are top-down.
- **A piece can be bigger than a tile.** `w`/`h` on a `STRUCTURES` row is its
  footprint lying flat, and `rot` 1 turns it a quarter. `tx, ty` stays the
  top-left tile, every covered tile points at the piece in `grid`, and `pos`
  is the middle of the footprint, so every "which piece is on this tile" and
  "how far is that piece" in the game needed no change. `can_place` checks
  every tile (bounds, terrain, pieces, props, and for a solid piece anyone
  under any of it); `make` and `_unlink` walk them all. A square piece's `rot`
  is always stored 0.
- **R turns it while placing.** Reload means nothing with a hammer out, so
  `main.gd` routes its key to `BuildBar.rotate` instead of adding a second
  binding on R that the controls screen would call a conflict; the bar shows
  the key only for a piece that turns. The turn rides `Intent.build_rot`, the
  wire's build tuple (a fifth element; four still decodes), the save's
  structure record, and the snapshot's `ro`. A guest's mirror clears whatever
  it still has under a new footprint before building it, and applies a
  diff's removals before its builds, each only to the piece anchored on that
  tile: the other way round, a wall's "gone" found the bed just built across
  its tile and took it, and the host never resends an unchanged bed (Codex,
  PR #50).
- **The Long Raised Bed (`longBed`)** is one planting over two tiles: one
  seed, one feed slot, one water meter, and `yield_mul` 2.5 on the harvest,
  for Wood 30 · Sticks 12 · Fiber 10 and 240 HP — cheaper than two beds,
  which cost 36/16/12 and pay 2x. `Farming.yield_mul` (the soil times the
  feed) is the one number the harvest and the panel's band both read. It is
  a bed in every other respect: not solid, not `protect`. The owner chose the
  numbers; they are in Notion under Items (Building).
- **How the files were made.** `tools/slice_world.py` (conda `deadline-art`)
  cuts the two contact sheets with `slice_icons.cut`, drops their captions by
  height (every line of text is under 40px, every picture over 70), crops
  each picture tight and keeps its shape, cuts the turret head as a square
  round its pivot, and pads `longBed` into a menu icon. The sources stay out
  of the repo, as the icon sheets did.
- `building_test` pins the footprint (both tiles, turned, refused on either
  tile, freed on salvage, saved turned); `farming_test` the long bed's harvest
  and that it beats two beds; `net_test` a guest building one turned and the
  mirror losing all of it; `icons_test` the art seam, the gate's second look
  and that every file in `art/world/` is named after a piece or a state. The
  smoke lays out every piece and photographs it (`structure_art`), then turns
  a long bed with R and places it with the mouse (`long_bed_ghost`).

### Crafting takes a moment, and shows it (2026-09-15)

The owner, playing: pressing CRAFT showed nothing, so it got pressed several
times. It had worked every time. The only signs were a HUD notice and a
sound, and **a full screen covers the HUD** — so the notice was never seen
from the one place anybody crafts.

- **One craft takes `PLAYER.craft_time` (1.5s)**, the same for every recipe,
  times the player's `craft_time_mul` (1.0; below is faster) — the stat a
  perk will shorten it with. Not a "long crafting timer" (pillar 1): it is
  a beat you can see.
- **`PlayerSim.crafting` is a channel** `{id, t, dur, bench, left}`, filled by
  `Crafting.tick` from the top of `PlayerSim.tick`. `Crafting.start` opens it
  after `status` passes; at the top, `Crafting.craft` — unchanged, still the
  whole of the rules — asks again, pays and hands it over, then the next of a
  batch starts. **Nothing is spent until an item is made**, so every way it
  stops costs the time and nothing else.
- **It stops when** you are hit (`Damage.damage_player`), go down, die, get
  into a car (Codex, #49), are
  parked or Lurch, when `status` stops passing — walk off a bench (its tier
  is asked of the world every step), run out of materials or room mid-batch,
  go into an instance — or on CANCEL. **It does not stop** when you walk, or
  close the screen: by-hand work goes on in your pocket.
- **The screen:** CRAFT becomes a filling bar ("CRAFTING HATCHET · 3 LEFT")
  that takes no presses — a second press is exactly the thing that must not
  cancel — with CANCEL where the stepper was. The card being made fills its
  strip. Under the button, for 2.5s, "✓ HATCHET CRAFTED" or "✕ CRAFTING
  INTERRUPTED" (`InventoryScreen.on_event` hears `crafted` and
  `craft_stopped` for its own seat). MEND and UPGRADE stay instant and get
  the same line. With the screen closed the HUD's prompt shows the bar.
- **Co-op:** `craft` opens the channel on the host and `cancel_craft` is new
  (protocol 8). The bar crosses as the player record's fifth string,
  `"axe:3:0.42"`, not the channel slot: you can reload while a craft runs.
  The event is keyed `by`, not `seat`, so a teammate beside the bench still
  hears it; the host sends a guest's pack the step its craft lands.
- `crafting_test` pins the time, one-at-a-time, batches, running out, the
  hit, cancel, walking off the bench and walking by hand; `net_test` that a
  guest sees the bar and its pack at once. The smoke asserts the bar, CANCEL
  and the line, and screenshots `craft_in_progress` and `craft_done`.
...
- **The street still draws structures in code.** The pictures are
  ...
  aim, a bed's crop growing, the power ring. Menu art only. *(Since
  2026-09-15 the street has top-down pictures of its own in `art/world/`,
  with that state drawn over them — see "Built pieces look like themselves".)*
...
The owner's handoff is `tasks/ui-redesign-handoff/` (moved there from the
repo root on 2026-09-14): a README that was the spec, a style sheet, and
eleven 1920x1080 mockups. It is
...
- **Nothing goes off screen, at any window size** (the owner, mid-build).
  ...
  trims rather than widening its column. A grid of cards is `Ui.card_grid`:
  as many `CARD_W` cards across as the column holds, the rest on the rows
  below, and a card's name and locked bill wrap inside it. The smoke run asserts it at every
...
- **The scene pumps the hub, every step, on both sides.** A `PeerHub` moves
  no bytes until `poll()`, and the sessions never call it: the host's step
  does (`_host_step`, and the menu step), and a guest's does
  (`_guest_step`, menu up or not). Until 2026-09-14 the guest's didn't —
...
- **One coloured multiply is the whole night**, and `DayNight.canvas_tint_at`
  is it: `lerp(WHITE, tint, alpha)` off `Config.DARKNESS_KEYS`, set on the
  `CanvasModulate` and nothing else. It was that multiply *plus* an additive
  `DirectionalLight2D` for `tint * alpha`, on the reasoning that an overlay is
  a multiply and an add and a CanvasModulate can only multiply. Godot's canvas
  shader saves `base_color` before the canvas modulate and every light pass
  multiplies by it, so that "add" was a second multiply — and it was a floor
  under how dark night could get. See the decision log, 2026-09-10.
- **A light in your off-hand strikes itself.** `Equipment.update_light` lights
  what you are wearing when the clock crosses `DARK_ENOUGH` and puts it out at
  first light, so `burn` is only ever spent in the dark — which is what makes
  210 seconds of torch mean one 213-second night. `T` is for going dark on
  purpose, `light_doused` is what makes that stick, and dawn forgives it so
  tomorrow night lights itself again.
...
- **`Crafting`** — the materials are the whole cost; since 2026-09-15 each
  one also takes a 1.5s bar you can see (§4, *Crafting takes a moment*). `C`
...
- **`Equipment`** — equip, unequip, quick-equip, moves, drops and the light,
  ...
  `update_light` is the only place the light's state changes with time: it
  strikes what you are wearing when the clock crosses `DARK_ENOUGH`, spends
  `burn` while it is dark and nowhere else, puts it out at first light, and
  takes a spent torch with it. It is called **above** `PlayerSim.tick`'s
  downed and driving branches, because it is a clock and not an action — a
  torch does not stop burning because you are bleeding out or behind a wheel.
  `toggle_light` is the *manual* half — going dark on purpose
  (`light_doused`, cleared at dawn) and spending a battery — and that one
  stays with the rest of the input, below those branches.
...
| 2026-09-10 | The night is one coloured multiply, and the additive half of it is deleted | The dark was a grey `CanvasModulate` for `1 - alpha` *plus* an additive `DirectionalLight2D` for `tint * alpha`, because an overlay is a multiply and an add and a CanvasModulate can only multiply. Godot's canvas shader (`canvas.glsl`) saves `base_color` **before** applying the canvas modulate and every light pass does `light_color.rgb *= base_color.rgb` — so that light added `tint * alpha * pixel`, a second multiply wearing an add's clothes. It never tinted a black pixel, which was its whole purpose, and it set a floor under the night: at #161436 and #070c1c the map still came through at a seventh of daylight, which is why the owner's second playtest "never got so dark that I couldn't see". Now `DayNight.canvas_tint_at` is `lerp(WHITE, tint, alpha)` — identical arithmetic to what the two nodes actually produced — one node instead of two, no whole-screen light pass a frame, and the darkness assertable in the fast suite. | Yes |
| 2026-09-10 | Deep night is black because the *tint* is near-black, not because alpha is 1 | Alpha was already 0.959, so `1 - alpha` was 4% and could not be the problem; the residual was the tint. The deep-night keys went to #04060f (about 6% of daylight on the brightest channel, under 3% against ground art), and the alphas were scaled by 1/0.959 so the curve peaks at exactly 1.0 with `DARKNESS_FULL` and `DARK_ENOUGH` scaled by the same factor — `k`, which spawns, sense, speed, Threat and Mutation read, is unchanged at every key, as it was for the 1.17 scaling before it. `playtest_test.gd` still holds it to the pre-scaling values. The tint is also interpolated now rather than stepping to the nearer key, which was invisible while it only tinted and is a step in what you can see now that it multiplies. | Yes, the keys |
...
| 2026-09-15 | Built pieces are drawn as the owner's top-down pictures, with their state drawn over them in code | The icon set's pictures were three-quarter views, so the street kept code shapes; these are top-down and the owner made them for the street. What a piece is doing — damage, a crop, a turret's aim, power — is still code, because a picture cannot show it. No picture still means the old drawing. | Yes, delete a file |
| 2026-09-15 | Walls and gates fill their tile and trim their outline where they meet; a gate turns to follow its neighbours rather than with R | The owner saw gaps between walls and a gate facing the wrong way. Trimming a stretched picture's edge per side keeps the art's outline round the outside of a wall, where an autotile set would have needed new art. Following the neighbours makes a gate right with no key to learn, and keeps "a square piece's `rot` is 0" true, so nothing outside `StructureView` changed. | Yes |
| 2026-09-15 | A piece may cover several tiles: `w`/`h` in data, `rot` on the piece, top-left anchor, every tile in `grid` | The owner asked for a two-tile bed now. Anchoring at the top-left and pointing every tile at one piece kept `at_tile`, `pos` and every save and wire key working unchanged, where a piece per tile would have needed linking and a shared HP. | Yes |
| 2026-09-15 | The Long Raised Bed is one planting with a 2.5x harvest for less than two beds cost, and R turns it | The owner's three calls, over two crops in one frame or a looks-only piece, and over horizontal-only. R is reload's key, borrowed while placing, so there is no second binding on it. | Yes, numbers in `data/` |
...
| 2026-09-15 | Crafting takes 1.5s, one number for every recipe, with `craft_time_mul` for a perk | The owner, after pressing CRAFT several times and seeing nothing; they chose the time, one speed for now, and a stat to tune later over per-recipe times. It amends "instant because the materials are the whole cost" without breaking pillar 1: the beat is feedback, not a chore. | Yes, one number |
| 2026-09-15 | Nothing is spent until an item is made; a hit, walking off the bench or CANCEL costs only the time | Spending at the start would need a refund path, and a refund can overflow a pack the way an output can. `Crafting.craft` asking again at the top of the bar keeps it the one place the rules live. | Yes |
| 2026-09-15 | You can walk while crafting, and being hit stops it; MEND and UPGRADE stay instant | The owner's call over rooting you like a bandage. Danger is the gate (pillar 2), not standing still. Mending is not what they asked about, so it keeps its instant action and gets the same confirmation line. | Yes |
| 2026-09-15 | While a craft runs, CRAFT is a bar that takes no presses, and CANCEL is its own button | Cancel on the same button would turn the exact double-press that started this into losing the craft. | Yes |
| 2026-09-15 | Loot needs a clear line to you, and a pile collides with walls as it moves | The owner saw loot magnetise through walls. Two causes: the pull and the pickup checked distance alone, and a pile moved by its velocity with no collision at all, so the pull slid it straight through brick. Both are fixed: the pull and the pickup need `has_line_of_sight` against the feet rule (structures included, so a built wall counts), and `Loot._slide_pickup` stops a pile one axis at a time against anything solid. Feet, not bullets: a pile does not float across a river or over a fence. A pile already inside something — nothing stops a wall going up on top of one — is put out beside it with `World.unstick`: from inside a wall there is no line of sight to anyone, so it would get no pull and lie there until it expired (Codex review, PR #48). | Yes |
...
0. **The owner walks, fights, and builds** (the Phase 1, 2 and 3 gates,
   ...
   **is night dark enough to matter and light enough to play in?** Twice
   asked and twice answered "darker": the curve now peaks at 1.0 with a
   near-black tint, which puts the map at about 6% of daylight and nothing
   at all past the edge of your torch. The open half is the other side of
   it — with a torch lit, is a 300px puddle enough to fight in, or is a night
   without a flashlight a night you should spend indoors?
...
### A scroll area that only scrolls down still takes its child's width

A `ScrollContainer` with horizontal scrolling *disabled* reports its child's
minimum width as its own. The craft grid counted its columns from the
scroller's width, so one card wider than 184px ("Stone Pickaxe" at a
workbench) widened the grid, the grid widened the scroller, the wider
scroller fit another column, and the bench screen settled at 4086px with its
CRAFT button two windows to the right. Every headless test passed, because the
loop needs a real layout pass to run. A column whose contents decide their own
count must not also take its width from them: `Ui.card_grid` leaves
horizontal scrolling on with no bar (`SCROLL_MODE_SHOW_NEVER`), and the smoke
photographs the bench's Weapons page.

### A loopback cannot see the pump

Co-op shipped with a test file of its own and a smoke leg, and the owner's first game
with a friend over the internet dropped the friend five seconds after they
arrived. The scene never polled a guest's hub once it was in the world, so
its ENet peer neither sent nor received, and ENet's timeout did the rest.
Nothing could see it: the tests pump hubs by hand, and the smoke joined its
guest through a `Loopback`, which needs no pump. **Whatever the real
transport needs from the caller has to be done by the caller under test** —
here, the scene, over a real socket (`_smoke_coop_over_udp`).
...
| Stamina Cost, Cleave | **Stamina Cost now has a field to land in; Cleave still does not.** `Stamina.swing_cost` reads the weapon's own `stam` and falls back to the flat `PLAYER.stam_swing` (2.0), so filling the Notion column in is a content sync and no longer a code change — measured first, because a flat cost makes the *heaviest* weapon the cheapest to fight with (a Sledgehammer clears ten walkers for 18% of the bar where a pipe costs 55%). Cleave is real but derived from the weapon's `arc` (`melee_targets` allows 6 targets over 1.4 radians, otherwise 3). |
...
| 2026-09-15 | **Built pieces look like themselves, and a bed two tiles long.** The owner's top-down pictures of every buildable are in `art/world/` (cut by `tools/slice_world.py`), drawn by `StructureView` fitted to the piece's tiles with damage, flash, crops, rings and labels over them; an open gate has its own picture and the turret's head turns on its mount; the build ghost shows the picture. Pieces can cover several tiles (`w`/`h`, `rot`, top-left anchor, every tile in `grid`), R turns one while placing, and the turn is in the intent, the wire, the save and the snapshot. New buildable: the Long Raised Bed (`longBed`), one planting over two tiles, harvest x2.5, Wood 30 · Sticks 12 · Fiber 10. Menu icons unchanged but for the new bed's. Tests in `building_test`, `farming_test`, `net_test`, `icons_test`; smoke `structure_art` and `long_bed_ghost`. **Codex pass (PR #50):** a guest applied a world diff's builds before its removals, so a bed built across a removed wall's tile was deleted by the wall's "gone" and stayed missing until a rejoin; removals now go first and only take the piece anchored there. `test_a_bed_built_over_a_removed_wall_survives_the_same_diff` fails without it |
| 2026-09-15 | **Crafting takes a moment, and shows it.** The owner pressed CRAFT several times because nothing seemed to happen: the notice went to the HUD, which the craft screen covers. Each craft is now a 1.5s channel (`PlayerSim.crafting`, `Crafting.start`/`tick`/`cancel`, `PLAYER.craft_time`, stat `craft_time_mul`), paid for when it lands, stopped by a hit, getting into a car, a bench you walk off or CANCEL. CRAFT becomes a filling bar with CANCEL beside it, the card fills its strip, a "✓ crafted" / "✕ interrupted" line sits under the button (mend and upgrade too), and the HUD prompt shows the bar with the screen closed. Co-op: protocol 8, `cancel_craft`, the bar as the player record's fifth string, the pack sent the step a guest's craft lands. Nine tests in `crafting_test`, a tighter `net_test`, and two smoke checkpoints (93) |
| 2026-09-15 | **Loot no longer comes through walls.** Ground piles were pulled and collected by distance alone and moved with no collision, so a pile on the far side of a wall slid through it to you. The magnet and the pickup now need line of sight (feet rule, built walls included), and a moving pile stops against anything solid. `test_loot_does_not_come_to_you_through_a_wall` fails against the old code (the pile ended up on your side). **Codex pass (PR #48):** a wall built on top of a pile buried it for good, since from inside the wall it could see no one to be pulled toward; a buried pile is now put out beside the wall, and `test_a_wall_built_on_a_pile_puts_it_out_beside_the_wall` fails without that. 739 tests. |
| 2026-09-15 | **The bench's CRAFT button stays on the window.** Playtest: at a workbench with plenty of materials the Weapons page ran off the right edge and the Compound Bow could not be crafted. A card wider than the 184px the column count assumed widened the grid, the grid widened its scroller (a `ScrollContainer` with horizontal scrolling disabled takes its child's minimum width), and the scroller's `resized` added a column — a loop that ended at 4086px. `Ui.card_grid` (craft page and build menu both) leaves horizontal scrolling on with no bar so the scroller's width is its column's, and counts columns against the widest card; card names and a locked card's bill are `Ui.para` and wrap, and the class line trims. The smoke run photographs the bench's Weapons page with the Compound Bow selected and asserts CRAFT is on the window (§8, *A scroll area that only scrolls down still takes its child's width*). |
| 2026-09-15 | **Stack counts show in the pack.** `UiSlot` drew the count in `Ui.INK`, the handoff's near-black for text on a pale swatch; once the icon set put art in the slot the number was dark on dark and nobody could read it. It is `TEXT_HIGH` over a `VOID` outline now, legible on pale and dark art. And a cell kept the container's own dictionary, so a count changed in place (a merge, a split, a pickup with the pack open) compared equal to itself and never redrew; it keeps a copy. `ui_review_test.gd`, failing against the old code first |
...
| 2026-09-11 | **Stamina stops refusing you and starts slowing you.** The old system turned work down: `can_chop` refused a harvest swing on a short bar, and *that refusal* was what set `winded` — sprinting never could, because `move` gated it at "stamina above 1" so the bar hovered just above empty and never reached zero (ported as found on 2026-09-08 and flagged for the owner's walk; this is the walk). Every part of that is gone. **Every swing costs now** — a fight, a harvest, and a swing at nothing, which used to be free — and **nothing is ever refused for want of puff**. Sprinting runs the bar flat. At zero you are **winded**, and winded costs you *time, never damage*: `swing_rate_mul` stretches a melee or tool swing by 60% and stretches the animation with it, because slowing the cooldown alone leaves a full-speed swing with a longer gap after it, which reads as lag rather than as fatigue. Damage per hit, yield per tree, walk speed and every gun are untouched — guns are outside the system entirely, since being tired should not spoil your aim, and so are crop plots and things picked off the ground, neither of which is swung at. **The debuff is a clock, not a lock** (`WINDED.dur`, 3.0s): recovery runs at the normal rate throughout, so three seconds of standing still hands back a usable bar, while swinging or sprinting restarts the clock — somebody who pushes through stays sluggish until they choose to stop. An earlier draft blocked regen for those three seconds and had to be thrown away: the debuff ended on an empty bar, so the next swing re-wound you instantly and the player got one swing every three seconds, flickering. `winded` reaches the stats through `recompute_stats` like a Mutation band (invariant 4) rather than being poked at the point of use, which is also what makes a guest agree with the host — the snapshot already ships it as one flag bit (`PF_WINDED`) and recomputes on arrival, so the penalty is *derived* at the far end rather than sent. `src/sim/stamina.gd` is the only writer of the bar, for the reason `Mutation.add` is (invariant 9): the flip has to happen somewhere that can recompute, and eight places used to write `stam`/`winded` directly. The winded clock ticks inside `move`, which a guest also runs to predict itself, so its own HUD countdown is right without the host shipping it. The meter turns `Ui.SHORT` rather than `Ui.DANGER` — DANGER is the health bar's own colour and the two meters sit adjacent, so a winded bar in it reads as a second health bar. Costs are unchanged in this pass (2.0 a swing, 6.0 a harvest via `stam_chop_mul` 3.0) so the feel can be walked before the numbers move, but every caller asks `Stamina.swing_cost(p, w)`, which reads the weapon's own `stam` first — §10's Stamina Cost rating now has somewhere to land. Measured while planning and worth keeping: a flat per-swing cost makes the heaviest weapon the most stamina-efficient one, and better tools are *three times* cheaper per tree, so upgrading is rewarded twice and the system erases itself; the ceiling (CON 2->10, Marathon x3, Fed) is 325 max and 53.5 regen, 3x the bar, against enemy HP that scales 24x from a walker to a Coach — which is the plan: **work graduates out of stamina, fighting never does**. `tools/test --all`: 753 tests, 17262 asserts, 0 failures. Smoke: 89 checkpoints, 1 failure, identical on `main` (the co-op guest-intent checkpoint, pre-existing and unrelated) **Review fix (2026-09-14):** a guest's mirror took the `PF_WINDED` bit as a bare assignment, so its own swing never slowed and its own `Stamina.tick` cleared the flag again a frame later; it now goes through `Stamina.sync`, which rebuilds the penalty and arms the countdown, and the predicted swing stretches by the same `swing_rate_mul` the host uses. `stam` is declared in `data/weapons.json`'s fields so the per-weapon cost really is a content sync |
