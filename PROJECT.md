...
- **R turns it while placing.** Reload means nothing with a hammer out, so
  ...
  structure record, and the snapshot's `ro` — protocol 9, because a
  protocol-8 host takes a tuple of exactly four and silently dropped every
  build a newer guest sent (Codex, PR #50). A guest's mirror clears whatever
...
- **You can see it: F2 in a dev build** (or *Toggle the noise overlay* in the
  F1 menu) draws each noise as a dashed circle
  ...
  so the circle has to sit still long enough to measure by eye. Only things
  that are swung, fired, built or driven make noise; walking and sprinting
  are silent, so a lens with nobody fighting shows nothing. It is drawn by
  `NoiseLensView` on its own camera-following CanvasLayer, because the night's
  CanvasModulate tints the world canvas and a ring drawn there went black. A
  guest sees no rings: it never ticks the sim, and the host never relays them.
