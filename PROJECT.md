...
| Unit tests | 560 tests, 7132 assertions (`tools\test.cmd`). `--all` adds the compound raid harness, the save round trips, the fire spread trials, the survivor combat tests, the UPnP door and the real broker under Node: 596 tests, 7276 assertions. The broker leg needs `npm install --prefix server` once per checkout — `server/node_modules/` is gitignored, so a fresh worktree does not have it and that test fails until it does. Wall-clock varies with the machine — see §9 |
