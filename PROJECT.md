...
- **Solo for now.** With anyone else present the door says so. PR D takes the
  whole party in together.
...
0. **Walk into the School.** PR B's gate. The dev menu has *Walk into Pine
   ...
   inside's dusk dark enough to want the torch. Then PR C, the phase boss,
   once the dash (PR A) has been felt.
