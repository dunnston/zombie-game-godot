...
- **A broken tool is not a tool.** A zero-condition Stone Knife does not cut
  cordage and a zero-condition Stone Hammer is not a portable workbench —
  "broken weapons do nothing until mended" has to reach the bench too. Nothing
  deadlocks: every recipe that names a tool is bench 0, and so is the recipe
  that mends the tool.
