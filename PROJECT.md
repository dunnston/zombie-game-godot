...
| Date | What |
| --- | --- |
| 2026-09-15 | **Loot no longer comes through walls.** Ground piles were pulled and collected by distance alone and moved with no collision, so a pile on the far side of a wall slid through it to you. The magnet and the pickup now need line of sight (feet rule, built walls included), and a moving pile stops against anything solid. `test_loot_does_not_come_to_you_through_a_wall` fails against the old code (the pile ended up on your side). 739 tests. |
